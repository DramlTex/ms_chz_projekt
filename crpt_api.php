<?php
// Общие помощники для работы с True API (НК/СУЗ)

if (!defined('TRUE_API_URL')) {
    define('TRUE_API_URL', 'https://markirovka.crpt.ru/api/v3/true-api');
}

if (!defined('NK_API_URL')) {
    define('NK_API_URL', 'https://markirovka.crpt.ru/api/v3');
}

if (!defined('SUZ_API_URL')) {
    define('SUZ_API_URL', 'https://suzgrid.crpt.ru/api/v3');
}

/**
 * Простейший файловый лог с маскированием чувствительных данных.
 */
 ...
 * При сетевых ошибках формирует подробный лог.
 */
function apiRequestRaw(string $url, string $method = 'GET', ?array $headers = null, $body = null): array
{
    ...
    if (defined('CURLOPT_DNS_CACHE_TIMEOUT')) {
        $options[CURLOPT_DNS_CACHE_TIMEOUT] = 300;
    }
...
/**
 * Выполнить HTTP-запрос и вернуть декодированный JSON.
 */
